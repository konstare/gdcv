...
#define DBG_NODETACH                                                           \
  (0 << 30 | 1 << 10)                  /* Don't detach as a background proc. */
...
static const char *_err_programName = "gdcv";
...
static int dict_read_header(const char *filename, dictData *header,
                            int computeCRC) {
  FILE *str;
  int id1, id2, si1, si2;
...
int dict_data_zip(FILE *inStr, const char *outFilename) {
  ...
  if (deflateInit2(&zStream, Z_BEST_COMPRESSION, Z_DEFLATED,
    ...
    err_internal(__func__, "Cannot initialize deflation engine: %s\n",
                 zStream.msg);
  ...
  if (st.st_size % chunkLength)
    ++chunks;
  PRINTF(DBG_VERBOSE,
         ("%lu chunks * %u per chunk = %lu (filesize = %lu)\n", chunks,
          chunkLength, chunks * chunkLength, (unsigned long)st.st_size));
  dataLength = chunks * 2;
  extraLength = 10 + dataLength;
  headerLength = GZ_FEXTRA_START + extraLength /* FEXTRA */
                 + strlen(origFilename) + 1;   /* FNAME  */
  PRINTF(DBG_VERBOSE, ("(data = %d, extra = %d, header = %d)\n", dataLength,
                       extraLength, headerLength));
...
char *dict_data_read_(dictData *h, unsigned long start, unsigned long size,  const char *preFilter, const char *postFilter) {
  ...
  switch (h->type) {
  case DICT_GZIP:
    err_fatal(__func__, "Cannot seek on pure gzip format files.\n"
  ...
  case DICT_DZIP:
    if (!h->initialized) {
      ...
      if (inflateInit2(&h->zStream, -15) != Z_OK)
        err_internal(__func__, "Cannot initialize inflation engine: %s\n",
                     h->zStream.msg);
    }
      ...
      if (i == firstChunk) {
        if (i == lastChunk) {
          ...
          if (count != h->chunkLength)
            err_internal(__func__, "Length = %d instead of %d\n", count,
                         h->chunkLength);
